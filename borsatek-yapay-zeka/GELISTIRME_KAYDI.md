...
|-------|-----|--------|------|
| 2026-05-05 | düzeltme | `tab-stream.php`, `borsatek-admin.js`, `class-admin.php`, `borsatek-admin.css`, `borsatek-yapay-zeka.php` v2.0.1 | **Odak kelime artık listede görünür:** Haber Akışı tablosunda **Odak kelime** sütununda metin kutusu + **Kaydet**. Çift önizleme bug’ı kaldırıldı (boş odakla yanlış AJAX). `borsatek_update_focus_keyword` nonce `borsatek_ajax` ile uyumlu; yetki `canAccessStream`. Toplu dönüştürmeden önce odak kelimeler sunucuya yazılıyor. Dönüştür POST ile gönderilen odak kelime meta’ya yazılıyor. |
